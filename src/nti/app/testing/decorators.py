...
def WithSharedApplicationMockDS(*args, **kwargs):
    ...
    if 'handle_changes' in kwargs:
        ...
        warnings.warn('handle_changes is always on',
                      DeprecationWarning, stacklevel=1)
        # needed to get the decorator vs factory logic correct below
        kwargs['with_changes'] = val
...
def WithSharedApplicationMockDSHandleChanges(*args, **kwargs):
    ...
    if len(args) == 1 and not kwargs:
        ...
        call_factory = True
    # yes, deprecated, but necessary to ensure kwargs not empty
